...
const Navbar = () => {
  ...
  return (
    <>
    <nav className="fixed top-0 left-0 right-0 bg-transparent backdrop-blur-sm shadow-sm z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className="flex-shrink-0">
            <h1 className="font-bold playfair text-gray-800">
            ...
            <div className="ml-10 flex items-center space-x-8">
              {navItems.map(({ id, label }) => (
                <button
                  ...
                  className="hover:text-[#ccabdb] transition-colors duration-300 relative group text-[16px] "
                >
                  {label}
            ...
            <div className="px-4 pt-2 pb-3 space-y-1 bg-white border-t">
              {navItems.map(({ id, label }) => (
                <button
                  key={id}
                  onClick={() => scrollToSection(id)}
                  className="block w-full text-left px-3 py-2 text-base  text-gray-700 hover:text-[#ccabdb] transition-colors duration-300"
                >
                  {label}
                </button>
              ))}
